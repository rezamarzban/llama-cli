...
Use Termux 0.118.3 from F-Droid (https://f-droid.org)
...
```
find /system /vendor -name libOpenCL.so
```
...
```
export LD_LIBRARY_PATH=~:$LD_LIBRARY_PATH  # Set env (add to ~/.bashrc: echo 'export LD_LIBRARY_PATH=~:$LD_LIBRARY_PATH' >> ~/.bashrc; source ~/.bashrc)
```
